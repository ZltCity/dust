...
VertexArray::VertexArray(const Buffer &vbo, std::span<const VertexAttrib> attributes_) : m_handle(create())
{
	attributes(vbo, attributes_);
}
...
void VertexArray::attributes(const Buffer &vbo, std::span<const VertexAttrib> attributes_)
{
	auto vbind = BindGuard(vbo);
	auto abind = BindGuard(*this);
	auto index = GLuint {};
